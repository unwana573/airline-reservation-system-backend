...
async def search_airports(db: AsyncSession, query: str) -> list[AirportSearchOut]:
    if len(query.strip()) < 2:
        return []  # avoid a near-full-table scan on a 1-character query
    airports = await content_repository.search_airports(db, query.strip())
    return [AirportSearchOut.model_validate(a) for a in airports]